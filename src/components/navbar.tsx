    ...
    >
      <TooltipProvider delayDuration={200} skipDelayDuration={300}>
        <Dock
          ...
          distance={isMobile ? 0 : 100}
        >
          {DATA.navbar.map((item) => {
            const isExternal = item.href.startsWith("http");

            return (
              <Tooltip key={item.href} open={!isMobile ? undefined : false}>
          ...
          {(() => {
            ...
            for (const [name, social] of Object.entries(DATA.contact.social)) {
              ...
              socialNavbarItems.push(
                <Tooltip key={`social-${name}`} open={!isMobile ? undefined : false}>
                  <TooltipTrigger asChild>
                    <a
                      href={social.url}
                      target={isExternal ? "_blank" : undefined}
          ...
          <Separator
            ...
            className="h-2/3 m-auto w-px bg-border"
          />
          <DockIcon className="rounded-3xl cursor-pointer size-full bg-background p-0 text-muted-foreground hover:text-foreground hover:bg-muted backdrop-blur-3xl border border-border">
            <LanguageToggle />
          </DockIcon>
          <DockIcon className="rounded-3xl cursor-pointer size-full bg-background p-0 text-muted-foreground hover:text-foreground hover:bg-muted backdrop-blur-3xl border border-border">
